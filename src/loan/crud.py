...
def _recalculate_repayment_risk_derivatives(summary: dict) -> dict:
    ...
    total_expected_repayment (get_total_expected_repayment) and total_unrecovered_repayment
    (get_total_unrecovered_repayment) are fetched directly by the caller on a due-date
    basis; total_collected_repayment is set by the caller to total_loan_principal_collected
    + total_admin_fee_collected (reporting-date basis, matching the Principal/Admin-Fee
    section — see get_repayment_risk_summary's docstring). This function only combines
    them into rates here: repayment_recovery_rate, unrecovered_rate, outstanding_rate, and
    delinquency_by_expected_repayment.
    ...
    ("Bad Debt Recovery") aware.

    delinquency_by_expected_repayment/delinquency_by_admin_fee are unrecovered-vs-
    disbursement ratios: total_unrecovered_repayment over total_disbursed_amount (matched
    ...
    # total_collected_repayment is set by the caller (total_loan_principal_collected +
    # total_admin_fee_collected), not derived here.
    ...
    # Performance section: delinquency ratios are unrecovered-vs-disbursement, not
    # unrecovered-vs-expected — see docstring above.
    summary["delinquency_by_expected_repayment"] = (
        (unrecovered / total_disbursed_amount) if total_disbursed_amount > 0 else 0
    )
    total_disbursed_plus_admin_fee = total_disbursed_amount + total_admin_fee_disbursed
...
def get_total_disbursed_amount(
    ...
    *,
    ...
    """total_disbursed_amount/total_admin_fee_disbursed, matched to /loan/coverage-
    utilization's total_disbursed_amount: SUM(td_loan.total_loan) (and SUM(td_loan.
    admin_fee)) for loans with loan_status IN (1,2,4), bucketed by disbursement month
    (l.proses_date). Unlike get_total_expected_repayment, loan_type=all here applies the
    ALL_LOAN_CONDITIONS union predicate (matching coverage-utilization's own loan_type=all
    handling) rather than an unfiltered total.

    Used as the denominator basis for delinquency_by_expected_repayment/delinquency_by_
    admin_fee in repayment-risk (see _recalculate_repayment_risk_derivatives)."""
    try:
        loan_conditions = resolve_loan_conditions(loan_type, db)
        query = """
        SELECT SUM(l.total_loan) as total_disbursed_amount, SUM(l.admin_fee) as total_admin_fee_disbursed
        FROM td_loan l
        {gmc_joins}
        WHERE l.loan_status IN (1, 2, 4)
        AND {loan_conditions}
        ...
        record = db.execute(text(query), params).fetchone()
        return {
            "total_disbursed_amount": record[0] if record and record[0] is not None else 0,
            "total_admin_fee_disbursed": record[1] if record and record[1] is not None else 0,
        }
    except Exception:
        import traceback
        traceback.print_exc()
        return {"total_disbursed_amount": 0, "total_admin_fee_disbursed": 0}


def get_total_disbursed_amount_monthly(
    db: Session,
    *,
    employer_filter: str = None,
    sourced_to_filter: str = None,
    ...
    """get_total_disbursed_amount, grouped by disbursement month_year."""
    try:
        loan_conditions = resolve_loan_conditions(loan_type, db)
        query = """
        SELECT DATE_FORMAT(l.proses_date, '%M %Y') as month_year,
               SUM(l.total_loan) as total_disbursed_amount,
               SUM(l.admin_fee) as total_admin_fee_disbursed
        FROM td_loan l
        {gmc_joins}
        WHERE l.loan_status IN (1, 2, 4)
        AND {loan_conditions}
        ...
        for row in db.execute(text(query), params).fetchall():
            if row[0] is None:
                continue
            monthly_data[row[0]] = {
                "total_disbursed_amount": row[1] if row[1] is not None else 0,
                "total_admin_fee_disbursed": row[2] if row[2] is not None else 0,
            }
        return monthly_data
    except Exception:
        import traceback
...
def get_repayment_risk_summary(db: Session,
    ...
    - total_expected_repayment / total_outstanding_repayment (and total_expected_repayment's
      own rate denominators) are due-date based and have nothing to do with bad debt: filtered
      /bucketed by raw due date (l.repayment_date for kasbon, tlh.due_date for extradana/
      aku_cicil) regardless of how late it was eventually paid. See get_total_expected_
      repayment's docstring — this matches how HRIS reconciles expected. total_unrecovered_
      repayment (get_total_unrecovered_repayment) is also computed on this due-date basis,
      but is numerically reporting-date-equivalent in practice: it only ever covers rows with
      no payment_date, and the reporting-date CASE below always falls back to the raw due
      date when payment_date is NULL, so the two bases agree for every row it counts.
    - total_collected_repayment / total_loan_principal_collected / total_admin_fee_collected
      and their unrecovered/expected counterparts (the principal-repayment and admin-fee-
      repayment metrics) count toward exactly one reporting period: their payment date if
      paid on/before the due date, otherwise their original due date. When start_date/
      end_date are given, these fields filter on that reporting date, not the raw due date.
      A row that crossed into Bad Debt Recovery (paid 3+ calendar months after its due month
      — see _BAD_DEBT_LUMP_PREDICATE/_INSTALLMENT) is excluded from total_loan_principal_
      collected/total_admin_fee_collected entirely (`AND NOT (bad_debt_predicate)`) — it
      shows up only via /loan/bad-debt-recovery, dated to its payment month, never both.
      total_collected_repayment is simply total_loan_principal_collected +
      total_admin_fee_collected (per explicit user decision to match the Principal/Admin-Fee
      section's month attribution instead of its own former due-date-based sum) — an
      installment paid early, before its due date, is now credited to its payment month here
      too, not its due month, so total_expected_repayment (due-date based) no longer equals
      total_collected_repayment + total_unrecovered_repayment + total_outstanding_repayment
      for a given period when early payments cross a month boundary; total_expected_repayment
      itself was deliberately left on the due-date basis (HRIS parity), so this divergence is
      expected, not a bug.
    ...
    try:
        ...
        if is_all_loan_types(loan_type):
            ...
            # See the per-product branch below: total_collected_repayment now derives from
            # the (already-merged) reporting-date + bad-debt-excluded principal/admin-fee
            # totals rather than its own due-date-based query.
            combined["total_collected_repayment"] = (
                combined["total_loan_principal_collected"] + combined["total_admin_fee_collected"]
            )
            ...
            disbursed_all = get_total_disbursed_amount(
                db,
                employer_filter=employer_filter,
                sourced_to_filter=sourced_to_filter,
                project_filter=project_filter,
            ...
            combined["total_disbursed_amount"] = disbursed_all["total_disbursed_amount"]
            combined["total_admin_fee_disbursed"] = disbursed_all["total_admin_fee_disbursed"]
        ...
        # user decision) instead of its own due-date sum — see this function's docstring.
        total_collected_repayment = total_loan_principal_collected + total_admin_fee_collected
        ...
        return _recalculate_repayment_risk_derivatives({
            ...
            "total_disbursed_amount": disbursed["total_disbursed_amount"],
            "total_admin_fee_disbursed": disbursed["total_admin_fee_disbursed"],
        })
...
def get_repayment_risk_monthly_summary(db: Session,
    ...
    summary (see that function's docstring): total_expected_repayment/unrecovered/
    outstanding (and their rates) are bucketed by raw due month (computed via
    get_total_expected_repayment_monthly) and have nothing to do with bad debt.
    total_collected_repayment/total_loan_principal_collected/total_admin_fee_collected
    (and the performance metrics derived from them) are bucketed by reporting month — the
    payment month if paid on/before its due date, otherwise its original due date (see
    _REPORTING_DATE_LUMP/_INSTALLMENT) — and total_loan_principal_collected/total_admin_
    fee_collected exclude rows that crossed into Bad Debt Recovery entirely (paid 3+
    calendar months late), which are reported only via /loan/bad-debt-recovery instead,
    never both; total_collected_repayment is simply their sum, so it inherits the same
    ...
    try:
        ...
        if is_all_loan_types(loan_type):
            ...
            disbursed_monthly = get_total_disbursed_amount_monthly(
                db,
                employer_filter=employer_filter,
                sourced_to_filter=sourced_to_filter,
                project_filter=project_filter,
            ...
            for month_year in all_months:
                ...
                # total_collected_repayment = total_loan_principal_collected +
                # total_admin_fee_collected, already summed across products in bucket by
                # _merge_monthly_repayment_risk above — see get_repayment_risk_summary's
                # docstring.
                bucket["total_collected_repayment"] = (
                    bucket["total_loan_principal_collected"] + bucket["total_admin_fee_collected"]
                )
        ...
        for month_year in all_months:
            ...
            # total_collected_repayment = total_loan_principal_collected +
            # total_admin_fee_collected for this reporting month (see
            # get_repayment_risk_summary's docstring).
            bucket["total_collected_repayment"] = (
                bucket["total_loan_principal_collected"] + bucket["total_admin_fee_collected"]
            )
